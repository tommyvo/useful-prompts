...
- **Content sync**: Body content should be identical across all four versions except for frontmatter and, where genuinely necessary, tool-specific instructions (e.g. `gh-pr-code-review`'s, `gh-pr-rails-review-checklist`'s, `gh-pr-react-review-checklist`'s, and `gh-pr-security-review-checklist`'s Copilot versions route PR metadata through VS Code's GitHub PR extension instead of raw `gh` CLI calls, because `gh pr diff` crashes VS Code — that's the exception, not the norm). The review and checklist skills' "How to launch them on this platform" paragraph (under "Large Reviews: Parallel Subagents") is also per-platform, because each tool launches subagents differently: Claude Code's `Agent` tool, Cursor's subagents, Copilot's `runSubagent` tool, and opencode's Task tool with the `general` subagent. Don't add `tools:` frontmatter to Copilot prompt files to enable subagents: it replaces the default tool list
