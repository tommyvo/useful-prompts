...
# Looks for skills in ~/.claude/skills/ that don't exist yet in this repo,
# imports them into "Claude Code/Skills/", and ports them to the other
# three platforms (Cursor, GitHub Copilot, Opencode).
...
while IFS= read -r -d '' dir; do
    ...
    target="$CLAUDE_SKILLS_DIR/$name/SKILL.md"
    if [ -f "$target" ]; then
    ...
    elif [ -f "$CURSOR_SKILLS_DIR/$name/SKILL.md" ] || [ -f "$COPILOT_PROMPTS_DIR/$name.prompt.md" ] || [ -f "$OPENCODE_COMMAND_DIR/$name.md" ]; then
        # Canonical Claude Code copy is missing, but a ported copy already
...
echo "  • Claude Code/Skills/<name>/SKILL.md   (canonical copy)"
echo "  • Cursor/Skills/<name>/SKILL.md        (identical shape)"
...
import_skill() {
    ...
    cp "$src" "$CLAUDE_SKILLS_DIR/$name/SKILL.md"
...
echo "  • Run ./install-cursor.sh, ./install-copilot.sh, ./install-opencode.sh"
