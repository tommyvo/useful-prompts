...
description: Review uncommitted changes against an application, secrets, supply chain, AWS/Terraform infrastructure, CI/CD, and optional SOC 2/GDPR security checklist and provide a prioritized report.
...
Provide a focused, deep security pass over uncommitted changes, checking them against the checklists below: application security, secrets, supply chain, infrastructure (Terraform/Atmos on AWS), CI/CD and containers, and optionally compliance and data handling. This complements `local-code-review`, which only covers security at a general level. The report lists security issues the developer should address, and suggests scanners to run.
...
2. **STEP 2 - Check Scope:** If the diff only changes documentation or formatting, output a single line saying there are no security-relevant changes to check, and stop. Otherwise continue - security applies to almost any code or configuration change.
3. **STEP 3 - Decide Which Checklists Apply:** Apply a checklist only when the diff touches what it covers:
...
5. You can read files in the current directory if you need more context (for example, the base Atmos stack a changed stack inherits from, or the policy attached to a changed IAM role).
6. When you give suggestions, please provide concrete example(s), and quote the code or configuration block.
7. Describe each finding in terms of risk: what an attacker or mistake could do, and under what conditions.
...
(Describe what the changes are about in one or two sentences, which checklists applied, and which compliance frameworks were in scope, e.g. "Secrets, Infrastructure (Atmos, ECS), CI/CD; compliance: SOC 2".)
...
1. File: (file 1 path)
   ...
   Line 3-5:
   (the risk: what could happen, and under what conditions)
...
(Note: Only include files that have specific issues. Omit files with no issues.)

# General Suggestions

(If there are issues that are related to many files. List them here)
...
(Only the commands relevant to this diff, if installed. Do not run them.)
...
- Security result: (No issues found/Minor issues/Issues need attention)
