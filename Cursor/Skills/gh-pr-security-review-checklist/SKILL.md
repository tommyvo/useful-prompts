---
name: gh-pr-security-review-checklist
description: Check a GitHub Pull Request against an application, secrets, supply chain, AWS/Terraform infrastructure, CI/CD, and optional SOC 2/GDPR security checklist using the gh CLI and provide a prioritized report.
disable-model-invocation: true
---

# GitHub PR Security Review Checklist

Review a GitHub Pull Request using the `gh` CLI and check it against the security checklists below: application security, secrets, supply chain, infrastructure (Terraform/Atmos on AWS), CI/CD and containers, and optionally compliance and data handling. This complements `gh-pr-code-review`, which only covers security at a general level.
...
- This skill is report-only. Do NOT modify any files, do NOT post comments or reviews to the PR, and do NOT run any scanners or other tools besides the read-only commands above
