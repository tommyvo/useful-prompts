...
Review a GitHub Pull Request using the `gh` CLI tool and check its JavaScript/TypeScript, React, and Next.js changes against the checklists below. This complements `gh-pr-code-review` (general correctness, security, clarity); it does not repeat that review.
